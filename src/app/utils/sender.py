...
class EmailCodeSender:
    ...
    def send_email(
        self, subject: str, email_to: str, html_template_name: str, body_params: dict[str, str]
    ):
        self._set_template()
        self.email.send(
            subject=subject,
...
class CodeSender:
    ...
    def _get_data_to_email(self):
        mail_subject, template = (
            ("Подтверждение электронной почты", "temp.html")
            if self.subject == "activate"
            else ("Восстановление пароля", "password_reset.html")
        )
